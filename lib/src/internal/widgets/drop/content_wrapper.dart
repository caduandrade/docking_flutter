...
abstract class ContentWrapperBase extends StatelessWidget{
  ...
  const ContentWrapperBase({Key? key, required this.layout, required this.listener, required this.child}) : super(key: key);
...
class ItemContentWrapper extends ContentWrapperBase {
  ...
  ItemContentWrapper({required DockingLayout layout, required DropWidgetListener listener,
    required DockingItem dockingItem, required Widget child}) :_dockingItem=dockingItem, super(layout: layout, listener: listener, child: child);

  final DockingItem _dockingItem;

  @override
  DropAnchorBaseWidget buildDropAnchor(DropPosition dropPosition) {
    return ItemDropAnchorWidget(layout: layout, listener: listener, dropPosition: dropPosition, dockingItem: _dockingItem);
  }
...
class TabsContentWrapper extends ContentWrapperBase {
  ...
  TabsContentWrapper({required DockingLayout layout, required DropWidgetListener listener,
    required DockingTabs dockingTabs, required Widget child}) :_dockingTabs=dockingTabs, super(layout: layout, listener: listener, child: child);

  final DockingTabs _dockingTabs;

  @override
  DropAnchorBaseWidget buildDropAnchor(DropPosition dropPosition) {
    return TabsDropAnchorWidget(layout: layout, listener: listener, dropPosition: dropPosition, dockingTabs: _dockingTabs);
  }
