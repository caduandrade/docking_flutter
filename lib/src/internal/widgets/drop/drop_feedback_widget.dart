...
class DropFeedbackWidget extends StatelessWidget {
  ...
  const DropFeedbackWidget({Key? key, this.dropPosition, required this.child}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return CustomPaint(foregroundPainter: _CustomPainter(dropPosition), child: child);
  } 
...
class _CustomPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if(dropPosition!=null) {
      Paint paint =Paint()..color = Colors.black.withOpacity(.5)..style=PaintingStyle.fill;
      late Rect rect;
